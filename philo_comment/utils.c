...
/*   Updated: 2021/11/09 22:16:52 by sonkang          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ph_init(int argc, char **argv, t_info **info, t_ph **ph)
{
	*info = (t_info *)malloc(sizeof(t_info));
	...
	if ((*info)->ph_num == 0 || (*info)->die_t == 0 || \
		(*info)->eat_t == 0 || (*info)->sleep_t == 0) //ft_atoi 에서 음수처리
			return (-1);
	(*info)->fork = (pthread_mutex_t *)malloc(sizeof(pthread_mutex_t) \
		* (*info)->ph_num); //fork 개수만큼 mutex 할당해줌
	*ph = (t_ph *)malloc(sizeof(t_ph) * (*info)->ph_num); //philo 개수만큼 구조체 할당
